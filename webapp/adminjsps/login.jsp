<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>

<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
  <head>
    <title>管理员登录页面</title>
    
	<meta http-equiv="pragma" content="no-cache">
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="expires" content="0">    
	<meta http-equiv="keywords" content="keyword1,keyword2,keyword3">
	<meta http-equiv="description" content="This is my page">
	<script type="text/javascript" src="<c:url value='/jquery/jquery-1.5.1.js'/>"></script>
	<script type="text/javascript" src="<c:url value='/js/input.js'></c:url>"></script>
	<link rel="stylesheet" type="text/css" href="<c:url value='/css/styles.css'/>">
	<style type="text/css">
		body{font-style: italic;}
	</style>
	<script type="text/javascript">
		$(function() {/*Map<String(Cookie名称),Cookie(Cookie本身)>*/
			// 获取cookie中的用户名
			var adminname = window.decodeURI("${cookie.adminname.value}");
			if("${requestScope.name}") {
				adminname = "${requestScope.name}";
			}
			$("#adminname").val(adminname);
		});
		function checkForm() {
			if(!$("#adminname").val()) {
				alert("管理员名称不能为空！");
				return false;
			}
			if(!$("#adminpwd").val()) {
				alert("管理员密码不能为空！");
				return false;
			}
			return true;
		}
	</script>
  </head>
  
  <body>
  <div style="margin: 100px">
	<div >
		<div style="float: left;">
			<a href="<c:url value='/index.jsp'></c:url>"><img src="<c:url value='/images/logoko.png'/>" /></a>
		</div>
		<div style="float: left;padding-top:80px; ">
			<h1>管理员登录页面</h1>
		 	<p style="font-weight: 900; color: red">${msg }</p>
			<form action="<c:url value='/AdminServlet'/>" method="post" onsubmit="return checkForm()" target="_top">
				<input type="hidden" name="oper" value="adminLoginByAdminNameAndAdminPwd"/>
				管理员账户：<input type="text" name="adminName" value="" id="adminname"/><br/>
				密　　　码：<input type="password" name="adminPwd" id="adminpwd"/><br/>
				<input type="submit" value="进入后台"/>
			</form>
		</div>
	</div>
  </div>
  </body>
</html>
